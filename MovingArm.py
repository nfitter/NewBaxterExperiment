...
class MovingArm(object):
    ...
    def __init__(self, rate, subj_num, trial_num):
        ...
        self._subjNum = subj_num
        self._trialNum = trial_num
    ...
    def _update_forces(self):
        ...
        if self._cubic:
            ...
            des_disp[self._joint] = -self._amplitude * np.sin(2 * np.pi * self._frequency * self._timeElapsed + 3*np.pi/8) + 0.86*self._amplitude
            des_vel[self._joint] = -self._amplitude * 2 * np.pi * np.cos(2 * np.pi * self._frequency * self._timeElapsed + 3*np.pi/8) + 0.86*self._amplitude
            des_acc[self._joint] = -self._amplitude * 4 * (np.pi ** 2) * np.sin(2 * np.pi * self._frequency * self._timeElapsed + 3*np.pi/8) + 0.86*self._amplitude
        ...
        file_name = 'Subject' + str(self._subjNum) + 'Pos' + str(self._trialNum) + '.csv' 
        f = open(file_name, "a")
        f.write(str(self._gtime) + ',' + str(cur_pos[self._joint]) + ',' + str(des_pos[self._joint]) + '\n')
        f.close()
